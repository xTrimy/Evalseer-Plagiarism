...
@section('content')
            ...
            <div class="w-full overflow-hidden rounded-lg shadow-xs">
              <div class="w-full overflow-x-auto">
                <table class="w-full whitespace-no-wrap">
                  ...
                  >
                  @php $i = 0; $x = 1;@endphp
                  @foreach ($submissions as $submission_group)
                     @php
                       $submission = $submission_group->last();
                     @endphp
                      <tr class="text-gray-700 dark:text-gray-400 text-center">
                      <td class="px-4 py-3">
                        <button class="caret-button" data-toggle="false" data-id="submission-{{ $i }}">
                          <i class="las la-caret-down text-black"></i>
                        </button>
                      </td>
                      <td class="px-4 py-3">
                    ...
                    @php
                      $submission_number = 1
                    @endphp
                    @foreach ($submission_group as $submission)
                    @if($submission == $submission_group->last())
                    @break
                    @endif
                    <tr class="max-h-0 hidden submission-{{ $i }}  overflow-hidden text-gray-700 dark:text-gray-400 text-center">
                      <td class="border-l-4 border-green-500">
                        {{ $submission_number++ }}
                      </td>
                      <td class="px-4 py-3  ">
                        <div class="flex items-center text-sm">
        ...
        <script>
          var caretButtons = document.querySelectorAll(
            ".caret-button"
          );
          for(let i = 0; i< caretButtons.length; i++){
            caretButtons[i].addEventListener("click", function(){
              this.getElementsByTagName('i')[0].classList.toggle("la-caret-up");
              this.getElementsByTagName('i')[0].classList.toggle("la-caret-down");
